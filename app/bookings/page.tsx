  ...
  return (
    <section className="space-y-6 lg:space-y-8">
      <div className="card flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="inline-flex items-center gap-2 rounded-full bg-brand-50 px-3 py-1 text-sm font-semibold text-brand-700 ring-1 ring-brand-100">Bookings</p>
          <h1 className="mt-3 text-4xl font-semibold tracking-tight text-slate-900 sm:text-5xl">My Bookings</h1>
          <p className="mt-4 text-slate-600">Signed in as {user.email ?? 'user'}.</p>
        </div>
      ...
      {error ? (
        ...
        <div className="card flex flex-col items-center justify-center gap-3 py-12 text-center text-slate-600">
          ...
          <p className="text-lg font-semibold text-slate-900">You have no bookings yet.</p>
          <p className="max-w-md text-sm text-slate-500">Search for flights and reserve your first trip to start building your travel history.</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {bookings.map((booking) => {
            ...
            const passenger = Array.isArray(booking.passengers)
              ...
              : booking.passengers
            const totalPrice = typeof booking.total_price === 'number' ? booking.total_price : Number(booking.total_price)
            const totalPriceLabel = Number.isFinite(totalPrice) ? `$${totalPrice.toFixed(2)}` : booking.total_price
            const isCancelled = booking.status === 'cancelled'

            return (
              <article key={booking.id} className={[
                'card transition',
