...
export default async function ConfirmationPage({
  ...
  if (!user) {
    redirect(
      `/login?next=${encodeURIComponent(`/confirmation/${params.bookingId}`)}`,
    );
  }

  const bookingSelect =
    "id, user_id, status, booked_at, total_price, pnr_code, flight_id, seat_id, flights(flight_no, origin, destination, departs_at, arrives_at, aircraft_type), seats(seat_number, class)";
  ...
  if (!booking) {
    ...
    if (latestBooking?.id) {
      redirect(`/confirmation/${latestBooking.id}`);
    }
  ...
  const flight = Array.isArray(bookingRow.flights)
    ? bookingRow.flights[0]
    : bookingRow.flights;
  const seat = Array.isArray(bookingRow.seats)
    ? bookingRow.seats[0]
    : bookingRow.seats;
    ...
    <section className="card overflow-hidden">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="rounded-3xl bg-brand-50 px-5 py-4 ring-1 ring-brand-100">
          ...
          <p className="mt-2 text-3xl font-extrabold tracking-[0.18em] text-slate-900">
            {bookingRow.pnr_code}
          </p>
        </div>
      </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">
                <Plane className="h-4 w-4" /> Booking ID
              </div>
            ...
            <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">
                <Ticket className="h-4 w-4" /> Total price
              </div>
              <p className="mt-2 text-xl font-semibold text-white">
                {typeof bookingRow.total_price === "number"
                  ? `$${bookingRow.total_price.toFixed(2)}`
                  : bookingRow.total_price}
              </p>
            </div>
            <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">
                <CalendarDays className="h-4 w-4" /> Status
              </div>
            ...
            <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">
                <FileText className="h-4 w-4" /> PNR code
              </div>
            ...
            <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">
                <Plane className="h-4 w-4" /> Latest flight
              </div>
              <p className="mt-2 text-sm font-medium text-white">
            ...
            <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">
                <Ticket className="h-4 w-4" /> Latest seat
              </div>
              <p className="mt-2 text-sm font-medium text-white">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-2 text-sm text-slate-600">
              ...
              <p>
                Departure:{" "}
                {flight?.departs_at ? formatDateTime(flight.departs_at) : "-"}
              </p>
              <p>
                Arrival:{" "}
                {flight?.arrives_at ? formatDateTime(flight.arrives_at) : "-"}
              </p>
              <p>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              ...
              <Link
                href="/search"
                className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:-translate-y-0.5 hover:border-brand-300 hover:text-brand-700"
              >
                Search Again
              </Link>
            </div>
